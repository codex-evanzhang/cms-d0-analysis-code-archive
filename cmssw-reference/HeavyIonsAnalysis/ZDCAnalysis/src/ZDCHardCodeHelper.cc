...
double ZDCHardCodeHelper::rechit_Energy_2023Offline_EM(const QIE10DataFrame& digi){
   HcalZDCDetId zdcid = digi.id();
   int zside = zdcid.zside();
   int channel = zdcid.channel();

   double gain = 0;
   if(zside < 0) { gain = EM_Gains[0][channel-1];}
   else { gain = EM_Gains[1][channel-1];}
   return((charge(digi, signalTs_) - charge(digi, noiseTs_))*gain*offline2023Scale(zside));
}

double ZDCHardCodeHelper::rechit_Energy_2023Offline_HAD(const QIE10DataFrame& digi){
   HcalZDCDetId zdcid = digi.id();
   int zside = zdcid.zside();
   int channel = zdcid.channel();

   double gain = 0;
   if(zside < 0) { gain = HAD_Gains[0][channel-1];}
   else { gain = HAD_Gains[1][channel-1];}
   return((charge(digi, signalTs_) - charge(digi, noiseTs_))*gain*offline2023Scale(zside));
}
