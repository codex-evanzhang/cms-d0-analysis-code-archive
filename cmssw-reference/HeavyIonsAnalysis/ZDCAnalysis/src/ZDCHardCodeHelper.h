...
public:
  ...
  double rechit_Energy_2023Offline_EM(const QIE10DataFrame& digi);
  double rechit_Energy_2023Offline_HAD(const QIE10DataFrame& digi);
...
// Match the 2026 official reforest of 2023 PbPb UPC data. The restored
// TS2-TS1 helper uses the documented side gains above, but same-event
// comparison to the official forest shows a stable side-dependent absolute
// scale on sumPlus/sumMinus. Keep this factor isolated so the correction is
// auditable and can be replaced if the exact historical backport appears.
double Offline2023Scale[2] = {0.876579, 0.915746};
...
double offline2023Scale(int zside) {
  if (zside < 0) return Offline2023Scale[0];
  return Offline2023Scale[1];
}
