...
std::vector<ThresholdRow> CandidateRows(const std::string &side,
  ...
  rows.push_back(MakeRow(side, "nominal_AN", nominal, fit, data, empty, "AN threshold"));
...
void WriteReadme(const char *outDir,
  ...
  out << "# Data-Driven ZDC 1n Threshold Derivation\n\n";
  ...
  out << "## Natural Rule Check\n\n";
  out << "The best current data-driven rule is to choose the lowest threshold where the local fitted 1n density is at least twice the local EmptyBX density, the integrated EmptyBX tail is at or below `5e-5`, and the fitted 1n efficiency remains at least `97.5%`. The final analysis threshold is then rounded to the nearest `100 GeV`.\n\n";
  out << "| Side | AN nominal | recommended raw | recommended rounded | Gaussian 1n efficiency | EmptyBX fake above raw |\n";
  out << "|---|---:|---:|---:|---:|---:|\n";
  ...
  out << "- The AN cut is not assumed by the fit; it is only drawn for comparison.\n";
}
}  // namespace
