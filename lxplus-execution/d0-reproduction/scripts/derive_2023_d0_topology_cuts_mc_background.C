...
void CollectCandidates(SampleTree &sample, bool signal, bool requireNominalEventTopology, Long64_t maxEvents,
  ...
  for (Long64_t entry = 0; entry < n; ++entry) {
    ...
    if (requireNominalEventTopology && !PassHfGap(sample, 1100.0, 1000.0, 9.2, 8.6)) continue;
...
void CollectMcBackgroundCandidates(SampleTree &sample, Long64_t maxEvents,
                                   std::vector<std::vector<Candidate>> &out,
                                   std::vector<int> &eventCounts) {
  const Long64_t n = sample.Entries(maxEvents);
  for (Long64_t entry = 0; entry < n; ++entry) {
    ...
    const size_t nCand = sample.CandidateCount();
    for (size_t i = 0; i < nCand; ++i) {
      if (TruthMatchedPrompt(sample, i)) continue;
      const bool keep = PassCandidateBaseline(sample, i, false, true);
      if (!keep) continue;
      const int bin = FindBin(sample.Dpt->at(i), sample.Dy->at(i));
      if (bin < 0) continue;
      out[bin].push_back(MakeCandidate(sample, i));
...
void ScanEventThresholds(SampleTree &data, SampleTree &control, Long64_t maxEvents, const std::string &outDir) {
  ...
  for (double p : hfPlus) {
    for (double m : hfMinus) {
      Long64_t dataPass = 0, controlPass = 0;
      for (const auto &row : dataEvents) {
        const bool gammaN = row.zdcMinus > 1000.0 && row.zdcPlus < 1100.0;
        const bool Ngamma = row.zdcPlus > 1100.0 && row.zdcMinus < 1000.0;
...
void ScanTopology(SampleTree &mc, Long64_t maxEvents, const std::string &outDir) {
  ...
  CollectMcBackgroundCandidates(mc, maxEvents, background, backgroundCount);
...
void WriteReadme(const std::string &outDir, Long64_t maxEvents,
  ...
  out << "- MC background: non-truth-matched candidates in the AN sidebands from the same MC input\n";
  out << "- data sidebands input ignored in this variant: `" << dataPath << "`\n";
  out << "- EBX/control input ignored in this variant: `" << controlPath << "`\n";
  ...
  out << "## Outputs\n\n";
  out << "- `d_topology_scan.tsv`: MC-signal/nonmatched-MC-sideband scan over D topology cuts.\n";
...
int derive_2023_d0_topology_cuts_mc_background(const char *signalPath,
                                               const char *dataPath,
  ...
  ScanTopology(signal, maxEvents, outDir);
