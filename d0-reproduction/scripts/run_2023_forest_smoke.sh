...
OUTDIR="${D0_REPRO_FOREST_SMOKE_OUTDIR:-/eos/user/e/evzhang/codex-eos/outputs/d0-reproduction/recreated-forest-2023/smoke}"
INPUT_FILE="${D0_REPRO_FOREST_SMOKE_INPUT:-root://xrootd-vanderbilt.sites.opensciencegrid.org//store/hidata/HIRun2023A/HIForward0/MINIAOD/16Jan2024-v1/2810000/3cb20bb5-a7b1-43a2-8025-9807f4f51e71.root}"
MAX_EVENTS=100
LABEL="hiforward0_miniaod_100"
ZDC_MODE="hc2023"
ZDC_VARIANT="default"
GLOBAL_TAG_OVERRIDE=""
DISABLE_MUONS=false
...
usage() {
  cat <<USAGE
Usage:
  run_2023_forest_smoke.sh [--max-events N] [--input-file ROOT_OR_XROOTD_PATH] [--label NAME] [--outdir PATH]
...
ReReco configuration. It writes only under Evan-owned CERN paths. It does not
submit CRAB or run full production.
...
branches read by the current 2023 D0 Section 5/7 mass-chain validation.
...

{
  ...
  cmsRun "$RUN_CFG"
  if [[ "$MINIMAL_ANALYSIS" == "true" ]]; then
    MINIMIZER="$BASE/scripts/minimize_2023_d0_forest.C"
    if [[ ! -f "$MINIMIZER" ]]; then
...
{
  printf '# 2023 Forest Smoke Reproduction: %s\n\n' "$LABEL"
  printf '<!-- DOC_OWNER: cms-analysis-manager forest smoke manifest. -->\n'
  ...
  printf -- '- Log: `%s`\n\n' "$LOG"
  printf '## Scope\n\n'
  printf -- '- This validates that the forest + embedded Dfinder configuration can run from MINIAOD.\n'
  printf -- '- It is not full production and is not a replacement for CRAB over HIForward0-19.\n'
} > "$MANIFEST"

{
  printf '%s\t%s\t%s\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "forest-smoke" "$LABEL" "$OUTPUT_ROOT"
