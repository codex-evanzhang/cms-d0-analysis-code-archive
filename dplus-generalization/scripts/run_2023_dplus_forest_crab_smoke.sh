...
TEMPLATE_CFG="${DPLUS_REPRO_CRAB_TEMPLATE_CFG:-/eos/user/e/evzhang/codex-eos/outputs/d0-reproduction/recreated-forest-2023/analysis-settings-smoke/hiforward0_analysis_settings_smoke_20260618/forest_smoke_cfg.py}"
LABEL="dplus_forest_crab_smoke_$(date -u +%Y%m%d_%H%M%S)"
STORAGE_SITE="T3_CH_CERNBOX"
OUT_LFN_BASE="/store/user/evzhang/codex-dplus-crab-smoke"
...
usage() {
  cat <<USAGE
Usage:
  run_2023_dplus_forest_crab_smoke.sh [--label NAME] [--storage-site SITE] [--out-lfn-base LFN]
...
forest config with Dfinder configured for D+/D- -> Kpipi. Splitting is
FileBased with unitsPerJob=1, so N input files produce N CRAB jobs. The default
uses two files to verify that behavior.
...

{
  ...
  printf 'config.Data.outputPrimaryDataset = "HIForward0DplusForestSmoke"\n'
...
{
  printf '# 2023 D+ Forest CRAB Smoke: %s\n\n' "$LABEL"
  printf '<!-- DOC_OWNER: cms-analysis-manager D+ CRAB smoke manifest. -->\n'
